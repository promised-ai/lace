...
fn get_cm_types(sub_m: &ArgMatches, ncols: usize) -> Vec<FType> {
    ...
    if use_default {
        ...
        if nflags != ncols {
            panic!(
                "Requested {} columns, but specified {} column types",
                ncols, nflags
            );
        }

        cm_flags.iter().for_each(|flag| match flag {
