...
mod tests {
    ...
    fn erf_of_very_large_value_should_be_1() {
        relative_eq!(erf(10.0), 1.0, epsilon = 1E-7);
    }
    ...
    fn erf_of_zero_should_be_zero() {
        relative_eq!(erf(0.0), 0.0, epsilon = 1E-7);
    }

    #[test]
    fn erf_negative_value_test_1() {
        relative_eq!(erf(-0.25), -0.2763263901682369, epsilon = 1E-6);
    }

    #[test]
    fn erf_negative_value_test_2() {
        relative_eq!(erf(-1.0), -0.84270079294971478, epsilon = 1E-6);
    }

    #[test]
    fn erf_positive_value_test_1() {
        relative_eq!(erf(0.446), 0.47178896844758522, epsilon = 1E-6);
    }

    #[test]
    fn erf_positive_value_test_2() {
        relative_eq!(erf(0.001), 0.0011283787909692363, epsilon = 1E-6);
    }
    ...
    fn erfinv_of_zero_should_be_zero() {
        relative_eq!(erfinv(0.0), 0.0, epsilon = 1E-7);
    }

    #[test]
    fn erfinv_positive_value_test_1() {
        relative_eq!(erfinv(0.5), 0.47693627620446982, epsilon = 1E-6);
    }

    #[test]
    fn erfinv_positive_value_test_2() {
        relative_eq!(erfinv(0.121), 0.10764782605515244, epsilon = 1E-6);
    }

    #[test]
    fn erfinv_negative_value_test_1() {
        relative_eq!(erfinv(-0.99), -1.8213863677184492, epsilon = 1E-5);
    }

    #[test]
    fn erfinv_negative_value_test_2() {
        relative_eq!(erfinv(-0.999), -2.3267537655135242, epsilon = 1E-4);
    }
}
