...
impl OracleError {
    ...
    pub fn to_string(&self) -> String {
        match &self {
            OracleError::RowIndexOutOfBounds { row_ix, nrows } => format!(
                "Row index {} out of bounds for oracle with {} rows",
                row_ix, nrows
            ),
            OracleError::ColumnIndexOutOfBounds { col_ix, ncols } => format!(
                "Column index {} out of bounds for oracle with {} rows",
                col_ix, ncols
            ),
            OracleError::InvalidDType {
                col_ix,
                ...
                "Column {} expects {} but recieved {}",
                col_ix, dtype, expected
            ),
            OracleError::GivenQueryColumnOverlap { col_ix } => format!(
                "Column {} appears in query and given conditions",
                col_ix
            ),
...
impl error::Error for OracleError {
    fn description(&self) -> &str {
        match &self {
            ...
            OracleError::NegativeEntropy { .. } => "negative entropy detected",
            OracleError::ZeroSamples => {
