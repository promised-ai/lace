...
pub trait GewekeModel: GewekeResampleData + GewekeSummarize {
    ...
    fn geweke_from_prior(settings: &Self::Settings, rng: &mut impl Rng) -> Self;
...
pub trait GewekeResampleData {
    type Settings;
    fn geweke_resample_data(&mut self, s: Option<&Self::Settings>, rng: &mut impl Rng);
}

pub trait GewekeSummarize: GewekeResampleData {
    fn geweke_summarize(&self, settings: &Self::Settings) -> BTreeMap<String, f64>;
}
