...
fn continuous_coltype(
    ...
    let (hyper, prior) = if no_hypers {
        (None, Some(crate::stats::prior::nix::from_data(&xs)))
    } else {
...
fn count_coltype(
    ...
    let (hyper, prior) = if no_hypers {
        (None, Some(crate::stats::prior::pg::from_data(&xs)))
    } else {
...
fn hyper_and_prior_for_categorical(
    no_hypers: bool,
    k: usize,
) -> (Option<CsdHyper>, Option<rv::prelude::SymmetricDirichlet>) {
...
fn rownames_from_index(id_srs: &Series) -> Result<RowNameList, CodebookError> {
    ...
    assert!(crate::utils::is_index_col(id_srs.name()));
...
pub fn codebook_from_csv<P: AsRef<Path>>(
    ...
    view_prior_process: Option<PriorProcess>,
    no_hypers: bool,
) -> Result<crate::codebook::Codebook, crate::codebook::CodebookError> {
