...
impl PriorProcessT for Dirichlet {
    ...
    fn update_params<R: Rng>(&mut self, asgn: &Assignment, rng: &mut R) -> f64 {
        ...
        let loglike =
            |alpha: &f64| crate::stats::assignment::lcrp(n, cts, *alpha);
        let prior_ref = &self.alpha_prior;
        let prior_draw = |rng: &mut R| prior_ref.draw(rng);
        let mh_result = crate::stats::mh::mh_prior(
            self.alpha, loglike, prior_draw, 100, rng,
        );
    ...
    fn ln_f_partition(&self, asgn: &Assignment) -> f64 {
        crate::stats::assignment::lcrp(asgn.len(), &asgn.counts, self.alpha)
    }
}
...
impl PriorProcessT for PitmanYor {
    ...
    fn update_params<R: Rng>(&mut self, asgn: &Assignment, rng: &mut R) -> f64 {
        ...
        let ln_f_alpha = {
            let loglike = |alpha: &f64| {
                crate::stats::assignment::lpyp(cts, *alpha, self.d)
            };
            let prior_ref = &self.alpha_prior;
            let prior_draw = |rng: &mut R| prior_ref.draw(rng);
            let mh_result = crate::stats::mh::mh_prior(
        ...
        let ln_f_d = {
            let loglike =
                |d: &f64| crate::stats::assignment::lpyp(cts, self.alpha, *d);
            let prior_ref = &self.d_prior;
            let prior_draw = |rng: &mut R| prior_ref.draw(rng);
            let mh_result = crate::stats::mh::mh_prior(
                self.d, loglike, prior_draw, 100, rng,
            );
            self.d = mh_result.x;
            mh_result.score_x
        };
    ...
    fn ln_f_partition(&self, asgn: &Assignment) -> f64 {
        crate::stats::assignment::lpyp(&asgn.counts, self.alpha, self.d)
    }
}
...
impl Default for Process {
    fn default() -> Self {
        Self::Dirichlet(Dirichlet {
            alpha: 1.0,
            alpha_prior: crate::consts::general_alpha_prior(),
        })
    }
}
    ...
    pub fn build(self) -> Result<PriorProcess, BuildPriorProcessError> {
        ...
        let process = self.process.unwrap_or_else(|| {
            Process::Dirichlet(Dirichlet::from_prior(
                crate::consts::general_alpha_prior(),
                &mut rng,
            ))
        });
